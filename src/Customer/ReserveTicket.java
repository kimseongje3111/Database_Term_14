...
public class ReserveTicket {
	...
	public void run(User user) {
		...
		for (ReservedSeat rs : rs_list) {
			...
			if (rs.isReserveBool()) {
				System.out.print(String.format("%6s", "[" + rs.getSeat()) + "] : X");
			} else {
				availcount++;
				System.out.print(String.format("%6s", "[" + rs.getSeat()) + "] : O");
			}
			count++;
		}
		System.out.println();
		...
		for (int i = 0; i < howManyReservation; i++) {
			...
			String seatNumber = this.inputString("> 원하는 좌석을 입력하세요. (ex.1) : ");
			for (ReservedSeat rs : rs_list) {
		...
		for (Ticket rt : result_ticket) {
			...
			for (ScreeningMovie sm : screening_list) {
				if (sm.getScreenDate().equals(rt.getScreenDate()) && sm.getMovieName().equals(rt.getMovieName())
	...
	private List<Ticket> internetPay(List<Ticket> list, User user) {
		...
		for (int i = 0; i < list.size(); i++) {
			System.out.println("> " + (i + 1) + "번째 표를 결제합니다.");
